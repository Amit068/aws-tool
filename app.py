...
if has_invoice and has_activity:
    ...
    st.info(f"📊 {len(activity_files)} Activity PDF(s) uploaded — Consolidated Invoice optional")
else:
    st.info("⬆️ Upload at least one PDF (Consolidated Invoice or Account Activity) to get started.")
...
has_any = has_invoice or has_activity

# Submit button — enabled when any PDF uploaded
...
# Block proceed if no PDF uploaded
if not has_any:
    st.stop()
...
kpi(c1, "🏢", "Total Accounts",  str(len(display_rows)), f"{len(invoice_files or []) + len(activity_files or [])} PDF(s)")
