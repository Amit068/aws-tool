...
def build_account_report_excel(rows):
    ...
    for svc in service_order:
        ...
        for r in svc_rows:
            ...
            for col, val in enumerate([r[h] for h in headers], 1):
    ...
    gl.font = t_font
    gl.fill = t_fill
    gl.alignment = Alignment(vertical='center')
    gl.border = bdr_med
    ...
    for i, svc in enumerate(sorted_svcs, 2):
        ...
        c1 = ws2.cell(row=i, column=1, value=svc)
        c1.font = d_font; c1.fill = fill; c1.border = bdr
...
if "aws_report_mode" not in st.session_state:
    st.session_state["aws_report_mode"] = "payer"

st.markdown('<div class="section-title">Select AWS Report</div>', unsafe_allow_html=True)
report_col1, report_col2 = st.columns(2)
with report_col1:
    if st.button("AWS Payer Accounts report", use_container_width=True):
        st.session_state["aws_report_mode"] = "payer"
with report_col2:
    if st.button("AWS Account report", use_container_width=True):
        st.session_state["aws_report_mode"] = "account"

selected_report_mode = st.session_state["aws_report_mode"]
mode_label = (
    "AWS Payer Accounts report"
    if selected_report_mode == "payer"
    else "AWS Account report"
)
st.info(f"Selected report: {mode_label}")

if not uploaded_files:
    st.session_state.pop("aws_parsed_invoice_data", None)
    st.session_state.pop("aws_uploaded_file_key", None)
    st.info("Upload your PDF files above, then click Submit to read them.")
    st.stop()

uploaded_file_key = tuple((f.name, getattr(f, "size", 0)) for f in uploaded_files)
...
if not submit_clicked and "aws_parsed_invoice_data" not in st.session_state:
    st.info("PDF selected hai. Data read karne ke liye Submit & Read PDF click karein.")
    st.stop()
...
if selected_report_mode == "account":
    if not account_detail_rows:
        st.error("No account line-item data extracted. Please check that the uploaded PDF has detailed AWS bill pages.")
        st.stop()

    df_account_report = pd.DataFrame(account_detail_rows)
    total_account_cost = df_account_report["Cost (USD)"].sum()
    top_account_service = (
        df_account_report.groupby("Service")["Cost (USD)"].sum().sort_values(ascending=False).index[0]
        if not df_account_report.empty else "N/A"
    )

    st.markdown('<div class="section-title">AWS Account report</div>', unsafe_allow_html=True)
    ac1, ac2, ac3, ac4 = st.columns(4)
    kpi(ac1, "📄", "PDF Files", str(len(uploaded_files)), "Detailed bill parser")
    kpi(ac2, "🧾", "Line Items", f"{len(df_account_report):,}", "Usage rows extracted")
    kpi(ac3, "💰", "Pre-Tax Cost", f"${total_account_cost:,.2f}", "Line-item total")
    kpi(ac4, "🔝", "Top Service", top_account_service[:16] + "..." if len(top_account_service) > 16 else top_account_service, "")

    service_summary = (
        df_account_report.groupby("Service", as_index=False)["Cost (USD)"]
        .sum()
        .sort_values("Cost (USD)", ascending=False)
    )

    st.markdown('<div class="section-title">Service Summary</div>', unsafe_allow_html=True)
    ...
    with right_col:
        pie_source = service_summary.head(10).copy()
        if len(service_summary) > 10:
            other_total = service_summary.iloc[10:]["Cost (USD)"].sum()
            pie_source = pd.concat(
                [pie_source, pd.DataFrame([{"Service": "Others", "Cost (USD)": other_total}])],
                ignore_index=True,
            )
        ...
        fig_account_pie.update_traces(textposition='inside', textinfo='percent+label')
        fig_account_pie.update_layout(
            paper_bgcolor='#ffffff',
            font=dict(color='#0D2B4E'),
    ...
    st.markdown('<div class="section-title">Account Line Items</div>', unsafe_allow_html=True)
    st.dataframe(
        df_account_report[
            ["Source PDF", "Service", "Region", "Sub Service", "Description", "Consumption", "Unit", "Cost (USD)"]
        ],
        use_container_width=True,
        hide_index=True,
        column_config={
            "Consumption": st.column_config.NumberColumn(format="%.2f"),
            "Cost (USD)": st.column_config.NumberColumn(format="$%.2f"),
        },
    )

    st.divider()
    st.markdown("### Download AWS Account Report")
    st.download_button(
