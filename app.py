    ...
    for block in blocks[1:]:
        ...
        for line in lines[1:]:
            ...
            if 'For line item details' in line or 'Account Activity Page' in line:
                # This footer repeats on EVERY page of a multi-page account
                # (once per page, not once per account) — e.g. it can appear
                # 3-4 times for one account whose detail section spans
                # several pages. Using `break` here stops parsing the account
                # entirely at the first page break, silently dropping every
                # service listed on later pages. `continue` just skips the
                # footer line itself and keeps reading the rest of this
                # account's block (page numbers / "Account number" repeat
                # lines that follow are harmless — they match no pattern).
                continue
