...
with col_m2:
    ...
    <div class="mode-card">
        ...
        <div class="mode-desc">
            Upload your AWS Bill Summary or Cost & Usage PDF.<br>
            Drill down into line-item details: service, region, consumption & cost.<br>
...
def _extract_pages_bill(pdf_hash: str, pdf_bytes: bytes) -> list:
    """Extract text per page from bill PDF — cached by MD5 hash."""
    pages = []
    with pdfplumber.open(BytesIO(pdf_bytes)) as pdf:
        for page in pdf.pages:
            t = page.extract_text(x_tolerance=2, y_tolerance=2)
...
@st.cache_data(show_spinner=False, max_entries=10)
def parse_bill_pdf(pdf_hash: str, pdf_bytes: bytes):
    rows = []
    current_service = "Unknown"
    current_region  = "Unknown"
    current_sub     = ""
    pages = _extract_pages_bill(pdf_hash, pdf_bytes)
    for text in pages:
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="section-title">📂 Upload Bill PDF — Service-wise Breakdown</div>',
                unsafe_allow_html=True)
    ...
    <div style="background:#FFF8EC;border-radius:10px;padding:10px 14px;
        ...
        <span style="color:#0D2B4E;font-size:12px;">
            Upload your AWS Bill Summary or Cost & Usage PDF.<br>
            Get granular line-item details per service: region, consumption & cost.<br>
            <b>Only one PDF per session — uploading multiple files will trigger an error.</b>
        </span>
    </div>
    """, unsafe_allow_html=True)

    bill_files = st.file_uploader(
        "Upload Bill PDF (single file only)",
        type=["pdf"],
        accept_multiple_files=True,
        key="mode2_uploader",
    ...
    if bill_files and len(bill_files) > 1:
        st.error(f"""
        ❌ **Only ONE PDF is allowed at a time in Mode 2!**

        You uploaded **{len(bill_files)} files**. Please remove the extras and keep only one PDF.

        To process multiple months, run them one at a time.
        """)
        st.stop()
    ...
    if has_bill:
        st.success(f"✅ {bill_files[0].name} — Ready to process")
    else:
        st.info("⬆️ Upload a single Bill PDF and click Submit to begin.")
    ...
    # ── Parse ──
    if submit_m2:
        with st.spinner("Parsing Bill PDF — extracting line items..."):
            try:
                pdf_bytes = bill_files[0].getvalue()
                pdf_hash  = _pdf_hash(pdf_bytes)
                rows = parse_bill_pdf(pdf_hash, pdf_bytes)
    ...
    if not rows:
        st.warning("⚠️ No line-item data found. Please verify you have uploaded a valid AWS Bill PDF.")
        st.stop()
    ...
    if "m2_excel" not in st.session_state or st.session_state.get("m2_excel_fname") != fname:
        with st.spinner("Building Excel report..."):
            st.session_state["m2_excel"] = build_bill_excel(rows)
            st.session_state["m2_excel_fname"] = fname
